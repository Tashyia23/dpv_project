...
if view_mode.startswith("Before"):
    ...
    with st.expander("📘 Insight – Top 30 Countries (Raw AQI)"):
        st.markdown(
            f"""
- **{selected_raw}** levels vary strongly across countries, showing **uneven exposure** to this pollutant.  
- Taller bars indicate countries with substantially **higher raw pollution** – often related to **urbanisation, industrialisation or traffic**.  
- The gap between the highest and lowest bars reflects **global inequality in air quality**.  
"""
        )
    ...
    with colX:
        ...
        with st.expander("📘 Insight – Histogram"):
            st.markdown(
                f"""
- The shape of this histogram shows how **{selected_raw}** is distributed across all records.  
- A **right-skewed** shape indicates that most locations have lower pollution, with a few **extreme high values**.  
- Outliers or heavy tails suggest **episodes or regions** with unusually high air pollution.  
"""
            )
    ...
    with colY:
        ...
        with st.expander("📘 Insight – Box Plot"):
            st.markdown(
                f"""
- The box plot summarises **median**, **spread**, and **outliers** for **{selected_raw}**.  
- A long upper whisker or many high outliers implies **extreme pollution values** in some countries.  
- A compact box shows that most locations have **similar pollution levels**, with fewer extremes.  
"""
            )
    ...
    with st.expander("📘 Insight – Correlation Heatmap"):
        st.markdown(
            """
- Strong positive correlations (red) indicate pollutants that **increase together**, often from **shared sources**  
  (e.g. traffic → PM2.5 & NO₂).  
- Weak or negative correlations suggest pollutants come from **different processes** or **different seasons**.  
- This helps identify **pollutant combinations** that may jointly drive health risks.  
"""
        )
    ...
    if sc_x == sc_y:
        ...
        with st.expander("📘 Insight – Scatterplot"):
            st.markdown(
                f"""
- This scatterplot shows how **{pretty_raw_labels[sc_x]}** and **{pretty_raw_labels[sc_y]}** move together.  
- A clear upward trend means both pollutants tend to **rise together**, hinting at **shared emission sources**.  
- A diffuse cloud with no pattern suggests the pollutants may be driven by **different activities or conditions**.  
"""
            )
    ...
    with st.expander("📘 Insight – WHO Health Impact (Raw)"):
        st.markdown(
            """
- Countries in **orange/red/dark red** exceed WHO thresholds by a wide margin, indicating **unsafe exposure levels**.  
- Countries in **green** are closer to or within WHO-recommended limits.  
- This view focuses on **health-based exceedance**, not just raw pollutant concentration.  
"""
        )
...
with st.expander("📘 Insight – WHO Health Impact (Processed)"):
    st.markdown(
        """
- This index converts pollutant data into a **health-focused metric**, based on WHO thresholds.  
- Values above **1.0** indicate exposure **beyond recommended safety limits**.  
- Compared to raw AQI, this view focuses on **exceedance of health standards**, not just raw values.  
"""
    )
...
for tab, level in zip([tab1, tab2, tab3, tab4], ["Very High", "High", "Moderate", "Low"]):
    with tab:
        ...
        if sub.empty:
            ...
            fig.update_xaxes(tickangle=45)
            st.plotly_chart(fig, use_container_width=True)
            with st.expander("📘 Insight – Category View"):
                st.markdown(
                    f"""
- Countries in the **{level}** category share similar **overall pollution burden**.  
- Higher bars indicate countries where **multiple pollutants** are simultaneously elevated.  
- This view supports **prioritisation** of regions for targeted policy and mitigation.  
"""
                )
            st.dataframe(sub)
...
with st.expander("📘 Insight – Composite Risk Map"):
    st.markdown(
        """
- The composite risk index combines **all selected pollutants** into a single score per country.  
- Countries in **orange/red** have **consistently higher levels** across several pollutants.  
- This highlights **multi-pollutant hotspots**, where people face the strongest combined exposure.  
"""
    )
...
with st.expander("📘 Additional Insight – Country Comparison"):
    st.markdown(
        f"""
- `{higher}` has a **higher overall pollution burden**, meaning several pollutants are elevated together.  
- The largest gap comes from **{key_pollutant}**, which is the **main driver** of the risk difference.  
- This comparison helps identify which pollutant should be **prioritised for intervention** in each country.  
"""
    )
...
with st.expander("📘 Insight – Country Ranking"):
    st.markdown(
        f"""
- This ranking highlights which countries are **most critical** for `{pollutant_selected}`.  
- A steep drop between bars suggests **large gaps in performance** between countries.  
- This view is useful for **benchmarking** and tracking whether high-risk countries improve over time.  
"""
    )
...
with st.expander("📘 Summary Insight"):
    st.markdown(
        """
- The **composite risk index** captures the *overall multi-pollutant burden* for each country.  
- The **WHO health-impact index** focuses on how far pollutant levels **exceed health-based thresholds**.  
- Used together, these indices provide both an **environmental** and a **health-focused** perspective on air quality risk.  
"""
    )
