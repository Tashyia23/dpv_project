...
view_mode = st.radio("View:", ["Before Processing", "After Processing"])

raw_g, raw_p = load_raw_dataset()
processed_df = load_merged_dataset()

if view_mode == "Before Processing":
    st.write("### 📄 RAW Global Dataset")
    st.dataframe(raw_g)

    st.write("### 📄 RAW PM2.5 Dataset")
    st.dataframe(raw_p)

    st.info("Raw data is displayed but cannot be mapped until after processing.")
    st.stop()

# Otherwise → use processed_df for the map
df = processed_df
